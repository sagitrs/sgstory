...
//注意：**一处定义**：本文件**不再自带**这份实现 → 用 `core/text.mjs` 的 `hasGeneratedMarker`
 ...
 *注意：JSON 无注释 →「剔注释只咬字段值」天然满足（`reason` 是字段）——同族口径 `test/multi-story.mjs` P6。 */
 ...
 *
 *注意：**为什么单列一族**（`#215` `18508333` 实测）：既有两个容器**都不能**装它 ——
 ...
 *
 *注意：**只咬字段值，绝不读散文字段**（本仓"留痕优先"）：本函数**不碰** `reason`／`why`／`paths`／`note`／
 * `refusedFacesWhy` 等任何一个散文面 → "`reason` 里写『与 `mist-forest` 同形』"（历史留痕）**不会被罚** ——
 * 这条**由构造保证**（不是靠关键词白名单，见 `test/k4-references.mjs` 的正例③）。
 *
 *注意：**`paths` 不属本条**（它**不是路径**）：`refusedFaces[].paths` 是**散文**（"三条路 (i)(ii)(iii)…"，实测现值）
 * → 对它做存在性判定＝**判了一件它不声称的事**（同族坑："过门 ≠ 达意"）。
 *
 *注意：**`proseFaces` 的存在性不重复判**（单一权威）：它已由 `handwrittenClosureProblems` 覆盖
 *（实测：往 `proseFaces` 注入不存在路径 → 该判据 rc=1 并点名"路径写错或文件已不在"）→ 本函数再判一次只会**重复报**。
 *
 *注意：**路径引用要求「已入库 ∩ 存在」**（`#1052`，单条引用的存在性判定）：
 ...
 *注意：**只咬文件引用**（`hatchFiles[]`／`refusedFaces[].file`）：`hatches[].slug` 判的是**目录**（故事集合）、
 * 口径是「故事目录现存」 → 不并入本条（并入＝改判它一件它不声称的事，同 `paths` 那条边界）。
 *注意：边界（`#1052` 记）：本门被判对象是**已入库的登记表** → **没有「漏扫整片」的面**，
 * 仅此一格（**单条引用**）→ 非阻断级。
 *
...
export const referenceIntegrityProblems = ({ registry = {}, slugSet = null, existsOf = () => false, trackedOf = () => true } = {}) => {
	...
	//注意：值只从**这三处**取（全是结构键）—— 散文字段一个都不进表。
	const rows = [
		...((registry.hatches ?? []).map((h, i) => ({
			at: `hatches[${i}].slug`, value: h?.slug, ok: (v) => slugs.has(v),
			//注意：`#1052` 的**入库**判定**只管文件类引用**：本条判的是**目录**（故事集合）→ `tracks` 恒真
	...
	for (const r of rows) {
		//注意：缺字段／空值**不归本条**（形式约束由各自的判据点名 —— 如 `refusedFaceProblems` 的四字段）
		...
		//注意：顺序不可反：先判磁盘 → 一个**被删且未入库**的文件报「不存在」（磁盘口径是真因），
		// 而不是报「未入库」（那会说错修法）。
		//注意：`tracks` 缺省为**恒真**（不是缺省为「跳过」）：缺省跳过 ＝ 新增行会**静默不判入库** →

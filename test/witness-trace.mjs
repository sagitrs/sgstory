...
//注意：本件**只读** `build/witness-trace.json`（gitignored）＋ **不并进** audit（报告型 同 scenarios 家的口径）。
...
if (r1.rc !== 0) bad.push(`① 见证器应 rc=0 ✗（实际 ${r1.rc}）\n${r1.out.slice(-500)}`);
	...
	//注意：**主判据是引擎的 `$pc.ev.ending`**（故事无关）—— 段落名前缀只兜底：
...
//注意：`#1004` B2b：**扫描面按样本重钉** —— 默认故事换成面夹具后，`seed=1` 单跑会在 40 步上限里
...
//注意：报文要点名**是哪一种不成立**：`K=99` 时头一个种子**是到得了结局的** → 报文该说"**到过结局、但都太短**"，
// 而不是笼统一句"没走到 ending"（那会把"轨迹太短"误读成"故事没有结局" → 也是本条自证的一格）。
else if (!/K=99/.test(r4.out)) bad.push(`③ 红得对、但报文没点名事件数 < K=99 ✗\n${r4.out.slice(-300)}`);
//注意：另钉一格：**K 一变大、扫描不能"遇到第一个结局就收工"** —— 若那样，这条会报"没走到 ending"而不是"太短"
...
// 改回前缀-only 的实现会在这里红）。
//注意：**声明**：**「段名不带「结局」前缀的结局样本」这一格自此无守护** → 日后要复钉它 → **先给某样本加一个
// 段名不带「结局」前缀、但由 `<<ending "…">>` 登记的结局**（注意：**不为凑绿改样本** —— 与 `test/rules-claims.mjs`、
// `ee67dcd` 那 4 件的退役同款；注意：改名的路已实测堵死：`src/80-script.twee:305` 与 `src/10-core.twee:413`
...
//注意：注意：③ 的负例跑完会把 `build/witness-trace.json` 覆盖成"**没命中**"的那份（`seed: null`、`steps: []`）
...
if (r5prod.rc !== 0) bad.push(`⑤ 产出真轨迹失败 ✗（rc=${r5prod.rc}）\n${r5prod.out.slice(-300)}`);
	...
	// ⑤-2注意：**轨迹刀**：改掉某一步的 `choiceKey`（改成**不存在的 key**）
	...
	// ⑤-2b注意：**向后兼容**：早先冻存件**没有 `endingKey`**（本片之前产的）→ 必须**仍能核过**
	...
	// 不带 `--story=` 时按**冻存件里记的 `story`** 核（否则会拿别的故事去默认故事上核 → 报"第 1 步不符" —— **误导**）。
	{
		...
		// ⑦-c注意：：冻存件**完全没有** `story` 键 → 按默认核 但要**点名**"这份没记"（不许静默）
		delete t7.story;
...
// ④注意：**绝对路径 → 不许静默降级**（本件实测口径，**不是**宣称它能跑通）

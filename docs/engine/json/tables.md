...
> ⚠️ **不是封闭集**：`night-ferry` 另有 `Shifts` 容器（本表未逐字段——按「容器由故事声明」口径，写故事时按需声明）；`night-ferry`／`minimal-demo` 各 12 个容器、无 `Codex`/`NPC`/`Star`/`Dragon`。照本表**全集照抄**会得到与两个既有故事都不同形的包 ✗。
...
> ⚠️ **编译器的键名是 `containers`**（写成 `rows` ⇒ `undefined` ⇒ **静默**产出一句空赋值 ✗）。
...
⚠️ `abil` 与 `skill` **都是检定**但读不同的表：`abil` 走属性调整值、`skill` 走技能加值 —— 混用会让减成不生效。
...
| 字段 | 型 | 含义 |
|---|---|---|
| `prices` | `{ [事件 id]: number }` | 显式价格表（⚠️ **运行时真源不读它**：`{price:…}` 实调 `Game.Economy.priceOf`（`src/engine/40-sim/21-resolve.twee:356`）＝事件 `delta` 经 `skillDiscount`/`featDiscount` 折扣后的值，**不读 `prices`**——反例：`drink_round` 的 `prices`=5 vs `delta`=−3 ⇒ 条件实值 3；`prices` 现由编辑器侧展示消费） |
...
⚠️ **实测形状与文档不一致（本仓已知漂移）**：`docs/story2-contracts.md` §1.2 声明的 `gearDef(id)` 是 `{kind,protects,maxHp,reduce,note}`；
...
⚠️ `items.effects` 与 `Gear.defs.advSites` **语义重叠**（同一件事两个家）—— 现状详见 `docs/engine/data-model.md` §四。
...
⚠️ `face-fixture` 的 `tables.json` 里 `Notes.entries` 是 `{}`，真数据在 `data/notes.json`（**EXTENSIONS**）。
...
⚠️ **声明面不得大于实现面**：`REDUCE_FORMS` 目前只有 `['flat']`；实现 `dice`／`percent` 时**同一 PR 三处同源**（本表 ＋ 引擎 ＋ 门）。

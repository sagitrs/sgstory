...
export const gearDefsCells = (t) => {
	...
	cell('🔴 Gear.defs 口径门·**完好态**：两面一致 ⇒ 0 问题（防"凡有差就报"✗）',
		gearDefsCriteriaProblems({ read: (f) => (f.startsWith('src/') ? ENGINE : '### 5.1 x\n\n| `damage` | n |\n\n## 6. y\n') }).length === 0);
	...
	cell('🔴 Gear.defs 口径门·抽取②：③ 侧只取 §5.1 那一段的表格首列（下一节的字段**不算** ✗）',
		(() => { const s = '### 5.1 甲\n\n| `a` | x |\n\n## 6. 乙\n\n| `b` | y |\n';
			return docDeclaredFields(s).join(',') === 'a'; })());
	cell('🔴 Gear.defs 口径门·**能假①**：给文档**加一个代码不读的字段** ⇒ 必报（对称差「③独有」非空 ✓）',
		(() => { const p = gearDefsCriteriaProblems({ read: (f) => (f.startsWith('src/') ? ENGINE : DOC.replace('## 9. 下一节', '| `ghost` | `x` |\n\n### 1.3')) });
			return p.length === 1 && /口径不一致/.test(p[0]); })());
	cell('🔴 Gear.defs 口径门·**能假②**：从文档**删一个代码在读的字段** ⇒ 必报（对称差「②独有」非空 ✓）',
		(() => { const p = gearDefsCriteriaProblems({ read: (f) => (f.startsWith('src/') ? ENGINE : DOC.replace('| `damage` | `number` |\n', '')) });

...
`Sg.rules.effects = ['yields', 'gives', 'sets']`。⚠️ **`sets` 写"真"不写值**（引擎 `applySets` 判 `readPath(pc, path) !== true`）。
...
| **J2** | **引用须已声明**：`{{名字}}` 的名字须在声明面内；`[[目标]]` 的目标须存在段落 | 未声明的取值名／悬空链接 ⇒ 红并点名 | 链接：既有 `integrity`／`event-graph`；取值：**缺** `#1048` | ⚠️ 半实现 |

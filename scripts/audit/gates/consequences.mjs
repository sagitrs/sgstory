...
if (wantAll || arg('consequences')) {
	...
	const { written, buckets, problems } = classifyNarrativeState({
		...
		Echoes: ctx.window?.Sg?.story?.echoes?.() ?? { list: [], revisit: [] },
	});
...
// `#1267` tail item 8: zero-story mode -- with no story there is no prose to judge,
...
export const run = (ctx) => {
	if (!ctx?.storySlug) {
		console.log("  #1267 零故事模式：无正文可判 ⇒ 本门未判（前提不成立）");
		return 0;
	}

...
//
//注意：**本件是「纯自证」**（`#1052` 起照录实测留痕）：正文①～⑥ 的输入是**内部常量**＋**注入面**
...
//注意：本仓**留痕优先** → `reason`／`why`／`paths` 里写"与 `mist-forest` 同形"**是应当允许的**
// —— 正例③ 就是钉这一格的（**它不是"顺带"**：这一格错了，门就会去罚历史留痕 → 假红）。
//注意：`paths` **不是路径**（是散文"三条路 (i)(ii)(iii)…"）→ 对它做存在性判定＝**判一件它不声称的事**。
//注意：`proseFaces` 的存在性**不重复判**：已由 `handwrittenClosureProblems` 覆盖（实测：注入不存在路径 → 该判据 rc=1 并点名）。
//
...
//注意：`#1052` 起本件**也**走真表（⑦ 主跑判据）：`git ls-files` ＋ 真 `escape-hatch.json`（**仍只读** 不写）。
//
// `#1052`：登记表里的**文件类**引用要求「**已入库 ∩ 存在**」（只判 `existsSync` → 未 `git add` 的新文件让
// 登记"看起来有效" —— `#1019`「没扫不许表现为通过」／`#1028`「`git add` 前跑是假绿」同一族）。
//注意：本节的两条打印件用**具名常量**：探针要能证明「未入库会报」↓ —— 掐掉收集那一路 → 这两行
...
//注意：本节**不是**合成例：真 `escape-hatch.json` ＋ 真 `git ls-files` ＋ 真 `existsSync`（宿主注入）。
//注意：它**在 `--selftest` 下也跑**（探针跑的就是 `--selftest` → 不跑则探针不咬 —— 见顶注）。
if (SELFTEST) {

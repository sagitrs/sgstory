...
export const judgeRoads = (ctx) => {
	...
	const __fn = requireCombatFace(Game, 'roadOffer');
	// **两态能假格（常驻）**：就绪与缺席必须给出不同判定；否则等于"门在跑但零判定"。
	{
		const readyInj = requireCombatFace({ Combat: { 'roadOffer': () => null } }, 'roadOffer') !== null;
		const absentInj = requireCombatFace({ Combat: {} }, 'roadOffer') === null;

...
//
// ⚠️ `#1004` B2b（**换样本** ✗ —— 发起者裁定 5740615725 第 2 条）：旧路线表 **20+ 条逐条以已删故事的结局命名** ✗
...
//   与 `--verify` 已冻结在仓 ✓）。
//   ⚠️ 本件**不摆造剧情** ✗、也不把判据改成面驱动 ✗（那是**掏空** ✓）：路线表按新样本的**真实分叉**写 ✓，
//   判据仍是「走到结局 ✓・无宏错误红框 ✓・不同屏重复骰面 ✓・结局页收尾 ✓・按 key 可点 ✓」。
//   ⚠️ 它**没有车卡**（`hasChargen()` ⇒ `false` ✓）⇒ 起始走它自己的开场段（`渡口` ✓），
//   与夹具的 `踏上旅途／快速成型／出发，前往歪脖子鸭酒馆` 三段链**无关** ✓（那道链只在 `chargen:true` 时走 ✓）。
//   ⚠️ 由此引起的两处阈值**重推**（不是调参 ✗）与一处棘轮**向下重签**（显式事件 ✗）写在 `scripts/report-rhythm.mjs`
...
const MILESTONE_PASSAGES = {     // 首个不可逆点（E4）
	// ⚠️ `#1004` B2b（换样本）：旧值 [`花田`('塔外花田') / `龙战`('龙·战','封印·并肩')] 是**旧故事**的首个不可逆点 ✓
...
// ⚠️ `#1004` B2b（换样本）：本函数**去掉了旧样本的"备药优先"偏好** ✗（那一手是旧故事的战斗池专属 ✓）——
...
// ⚠️ 它**没有车卡** ⇒ 一律 `newGame(random, 0, { story: 'night-ferry', chargen: false })` ✓。
...
//   ⚠️ 换样本后，渲染侧是**默认故事（＝面夹具，27 格 ✓）**、交互侧若只跑 `night-ferry` ⇒ 两侧**不是同一个故事** ✓
...
// ⚠️ 标 `synthetic` ✗：它在段落之间**瞬移**（`Engine.play`）⇒ 不是"玩家游玩路线" ✓ ⇒ 不入 C4/E4 样本 ✓
//   （与旧口径同：构造性用例只供覆盖门 ✓）。
...
//    ⚠️ 这是本片里**最通用**的一条判据 ✗：它与"哪个故事"无关 ✓ —— 任何故事的每个结局页都必须
...
//    ⚠️ 为什么独立于路线 1/2 ✗：那两条也会顺带走到 ✓ —— 但它们是**完整路线样本** ✓，

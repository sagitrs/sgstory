...
//注意：**本件做不到的一件事（如实写明）**：上游 `core/story.mjs` 的 `readStoryPackage` 把"**文件不在**"与
 ...
 *
 *注意：**它只对形状敏感**：值的改动（新增一行规则、改一个 `default`）**不动形状串** —— 这是**故意的**
 ...
 *注意：只用它做"人看的短号"与"变了没"的快速判；**不要**拿它当"同一方言"的**唯一**证据

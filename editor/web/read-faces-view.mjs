...
//
//注意：**适用面写清**（车道 E 判据 ②；发起者 `18504552` 逐条批）：`--reads` 门分**三级**，页内**跑得起的只有两级的一半** ——
...
//注意：**机制面（`src/**`）不在页内**：用户选的是**故事目录** → `src/engine/**` 那些 twee 不在取件面内（**如实说**，不假装覆盖）。
// → 读数里写的是"**① 同判 ／②③ 只列不判 **"（㉑／㉕）—— **不把"部分"报成"全部"**。
//
//注意：**覆盖率那条 `已接线` 是模块身份级代理**（`scripts/report-page-coverage.mjs` 件头自陈"必要不充分"）：

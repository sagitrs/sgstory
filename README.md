...
npm test        # 构建后全链（~2min）：L0 静态门（含表一致性）→ 五维质量门 → 规则/属性单测 → L1 全段渲染 → 冒烟 → 场景(19 路线并行) → 覆盖门
...
scripts/audit.mjs   质量五门（主伞 #34）：真相可达性/选择意义感/系统可玩性/世界活性/语言经济
                    + 数值三件套（检定成功率矩阵 / 经济时间线 / 龙战推演与道具伤害矩阵）——改叙事文本断锚即红
docs/
...
| 检定驱动剧情 | `酒馆/森林边缘/洞穴/雾之魔物/书房/工坊/温室/天文台/观星者/老巫女` | 技能检定/豁免代替裸随机（`<<sitecheck>>`） |
| 资源经济 | `酒馆/女巫小屋/洞穴/书房/温室/工坊/天文台/龙·巢边` | 金币买信息与准备；学识烙印与技能折扣 |
| 情报=优势 | `酒馆→洞穴`、`守林人` | 传闻给检定优势；带证物分层解锁守林人回答 |
| 道德分支 | `洞穴` | 买路 / 动武 / 绕开 → 一章回响（温室） |

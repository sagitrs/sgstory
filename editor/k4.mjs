...
export const contractSourceText = (files) => {
	const hand = files.filter(([name, text]) => name.endsWith('.twee') && !/@generated/.test(text));
	return { text: hand.map(([, t]) => t).join('\n'), handCount: hand.length, generatedCount: files.length - hand.length };
};
...
if (process.argv.includes('--selfcheck')) {
	...
	console.log('\n✔ 自证通过（12 条：标记 2 ＋ 新鲜度 2 ＋ 逃生舱双向 5 ＋ 手写源口径 3）');
	process.exit(0);
}
...
console.log('══ K4 门（`#762` 车道 C）—— 生成物标记 · 新鲜度 · 逃生舱可枚举 ══');
{
	...
	for (const slug of slugs) {
		...
		const { text: srcText, handCount, generatedCount } = contractSourceText(readdirSync(storyDir).filter((f) => statSync(join(storyDir, f)).isFile()).map((f) => [f, readFileSync(join(storyDir, f), 'utf8')]));
		if (!handCount) { console.log(`  · ${slug}：契约**已无手写源**（${generatedCount} 个产物文件）⇒ 声明面全在 data/（逃生舱由编译器的 kind 白名单把关）⇒ C 桶结构性为 0`); }
		...
		if (handCount > 0 && classified.length === 0) { console.error(`  ✗ ${slug}：手写契约源非空（${handCount} 文件）却分类出 **0 名成员** ⇒ 判据失效（不是通过）`); bad++; continue; }
		...
		console.log(`  · ${slug}：契约源 ${srcText.length}B（手写 ${handCount} 文件／产物 ${generatedCount} 文件）· ${dataReady ? '标记 ✓ · 幂等 ✓ · ' : '①/② 跳过（无 data/）· '}分类 A${buckets.A ?? 0}/B${buckets.B ?? 0}/C${buckets.C ?? 0}/D${buckets.D ?? 0}`);

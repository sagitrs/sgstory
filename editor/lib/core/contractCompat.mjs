...
//注意：**仓外**的旧包（浏览器里存的、导出的文件）**不可机检** → 那一半**靠 `ticket`／`reason` 的人审锚**；
//注意：「**一个发布周期**」**判不了**（经 `18503703` 明批）→ 落成 `reason ＋ ticket ＋ 删除计划`，
// **本件不假装能判它**。
//
//注意：**声明二附（`{ external: …}` 的来由 —— 量出来的，不是想出来的）**：
 ...
 *注意：这是"上限"那半的**唯一**出口 —— 想读更旧 → 走 `requireCompatVersion()` 的抛。 */
 ...
 * 而条目**还在** → **该删了** → 红。
 *注意：**只判仓内那一半**（仓外的旧包不可机检 —— 见件头声明二③）；`ticket` 是人审锚。 */

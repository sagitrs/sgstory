...
// 三态：① **缺面 ⇒ 内置**（＝今天常量 ⇒ 零变化）② **给覆盖 ⇒ 生效**（键级合并）③ **能假**（改包 ⇒ 读数跟着变）。
{
	...
	// ★★ 形态来源（**接续，✗ 不同源**）：首现于 **T 对 `#1463` 的 CR**（`roll(adv)` 硬编 `d20` 而 `d20(adv)` 已读包）——
	//    写作者把它提升为**通用形态**：「**点名一处该读包 ⇒ 必须扫"同概念还有没有别的入口"**」✓
	//    ⇒ 本格把那种形态**钉死**：`d20(adv)`（裸值）与 `roll(adv)`（带过程）**是同一概念（掷一颗骰）的两入口** ✓
	//
	// ★★ 判据写法（**照 T 的修法**，✗ 我第一版不行）：**确定性源 ＋ 合取** ——
	//    · **确定性源**：`R.rng.set((lo, hi) => hi)`（**恒取上界**）⇒ 读数可预测 ⇒ ✗ 不靠"跑 80 次碰运气" ✓
	//    · **合取**（`&&`，✗ 不是 `||`）：★我第一版用 `||` 合并两入口 ⇒ **单侧硬编也能全绿** ✗
	//      （写作者实锤：改回 `const S=20` 仍绿）⇒ 两态可分：正常 ⇒ `&&` 真｜**任一侧硬编 20** ⇒ `&&` 假 ⇒ **当场红** ✓
	{
		w.Sg.story.rulesPack = () => ({ dice: { sides: 100 } });
		R.rng.set((lo, hi) => hi);              // ★恒取上界 ⇒ 正常应得 100（任一侧硬编 20 ⇒ 得 20 ⇒ 红）
		const a = R.d20(0), b = R.roll(0);
		R.rng.reset();
		ok(a === 100 && b.roll === 100,
			`★ 两入口**各自**取到上界 100（合取 ⇒ 单侧硬编即红 ✗）：d20=${a}／roll=${b.roll}`);
		w.Sg.story.rulesPack = keep;
	}
}

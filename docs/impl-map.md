...
| `test/scenarios.mjs` | **30 条路线**（金路径 + 全部结局 + 设定集 + 龙巢边 + 时代分叉 + 封印战 + 反 S/L + 星力软限 + 结局页收尾 + **女巫小屋·只治一次**） | 重写 |
...
**当前规模**：`npm test` 全绿（**30 条路线** · 内容段 **66** · 渲染 **76** · 交互 **80** · `audit --text` 载荷标注 66 · 叙事段去标记 ≈1.85 万字）；`npm run soak`（游走 20+20 局 + **63 位点**双支清扫）0 违法，**已接回 CI 独立 job**（`deploy` 依赖 `[test, soak]`）。

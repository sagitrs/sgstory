...
**严格行为化率（有自证）：73/115 = 63.5%** ｜ **有断言但缺自证：41**（＝下方工作清单）｜ 仅登记：0
**探针（直接读数 ✓，不是"文件在不在"那种代理 ✗）：`✅` 32 项 ｜ `—` 未探 83 项（**上限 117** ✓ 超过即红 ✗；**调高它**是一次显式手改 ⇒ 靠评审拦 ✗，机器拦不住“手改上限”本身 ✓ —— 边界记在票 #908 内 ✗）｜ `✗` 不咬 0 项（**>0 即红** ✓）** —— 档位／清单：`node scripts/probe-gates.mjs --probe=fast` ✓（⑲：本轮覆盖到哪一档写在这行里 ✓）

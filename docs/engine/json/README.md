...
stories/<slug>/
  ...
  data/
    ...
    contract.json    ← 接入契约（`face-fixture` **25** 个成员） → 手册 §7
    chargen.json     ← 车卡数据集（可缺）          → 手册 §10
    meta.json        ← 元信息源（入口件的源）       → 手册 §3
...
**源**：`00-story.json` · `data/*.json`（六件，后四件可缺）· `passages/*.md`（＋ `audit.json`／`gates/` 属判据面）；**其余是产物**。
注意 **`00-meta.twee` 自 `#1132` B4 起是产物**（源为 `data/meta.json`；此前是唯一的手写 `.twee`），即 见 §3。
**`.twee` 一律是产物**（带 `@generated` 标记 ，即 K4 判据；手改会在下次编译被覆盖）。

**散文层已落地**（来源：`#1114` 片 2b-2b／片 3 与 `#1175`）：三故事（`face-fixture`／`night-ferry`／`minimal-demo`）的叙事段**全部**在 `passages/*.md`；段落拼装由 `build.mjs` 走 `editor/lib/core/passages.mjs` 的单一分派点（`.twee` 与 `.md` 同源）。，即 今天手写的"正文"写 `passages/*.md`，**不再写 `.twee`**。
...
```jsonc
// data/tables.json     注意：是 containers（写 rows ，即 undefined ，即 静默产出空赋值 ）
...
注意 `00-meta.twee` **必须带新 IFID**（照抄既有故事会撞；`build.mjs` 的 extwee 只收**大写** hex，小写 ，即 `Story IFID is invalid!` ，即 rc=1）。
...
- 数据面文件名**固定六个**：`tables.json` · `contract.json` · `rules.json` · `notes.json` · `chargen.json` · `meta.json`（后四件**可缺**；`rules.json`／`notes.json` 缺 ，即 读作 `null`，**不是**静默空对象）。
- `notes.json` 是**登记过的 EXTENSION**（`editor/lib/core/contractVersion.mjs` 的 `EXTENSIONS`）：ADD 新面要登记，**改既有面语义 ，即 `CURRENT + 1`**。

## 五、校验链（每个文件被谁咬）

| 文件 | 主要闸门 |
|---|---|
| `00-story.json` | `build.mjs`（故事件不在清单里 ，即 拒；清单里的文件不存在 ，即 拒）· `test/store-keys.mjs`（`slug` 与 `00-meta.twee` 的 `Sg.storyId` 一致）· `audience` **fail-loud**（缺字段／取值非法 ，即 报错） |
...
| `data/rules.json` | 🔴 **未实现**：`--rules` 开关不存在（实调 ，即 未知开关）；键形判据在 `test/cond-keyform.mjs`；死行判定在编辑器 UI（`ruleRows.mjs`）——详见 `reference-spec.md` §3.0 |
| 正文 | `test/prose-vocabulary.mjs`（禁逻辑宏 · 未宣告宏 ，即 红）· `scripts/md-format.mjs`（围栏／路径／表格） |

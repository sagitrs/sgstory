...
export const SEGMENTS = [
	...
	// `#1174` RC②：人类面行文门。**两段**：自证（判据函数）＋ **全仓实检**。
	// 为什么必须两段：只接自证时，往仓里注入一枚装饰记号不会红（自证只验函数、不看仓），已实测。
...
export const INPUTS_WILDCARD_REASONS = {
	...
	'scripts-lint-human-face-mjs-check': {
		reason: '本段是门的主体：逐件扫描全仓人类面（注释与文档），有装饰记号即红 => 面即整个仓库 => 取全跑型（否则存量记号会静默溜过）',
		voucher: '#1174',
	},
	'scripts-lint-human-face-mjs-selftest': {
		reason: '本段验判据函数自身（mask 定位、模板串不误判、代码区不动、双箭头残留为零、V3 全量机械验证）=> 不读仓面 => 仍取全跑型以免静默跳过成假绿面',
		voucher: '#1174',
	},

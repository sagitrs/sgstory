...
//   · 甲案（领队裁定，2026-09-22）：删记号（对勾、叉、星、表情），警号换「注意：」，
//     **箭头一律不文字化**，双箭头 `⇒` 降为单箭头 `→`，并规整记号删除后留下的标点邻域空白。
//
...
export const cleanText = (text) => String(text)
	...
	.replace(/\s*\u26A0\uFE0F?\s*/gu, '注意：')
	// `#1174` RC①：本函数**不做任何箭头文字化**（"按行内箭头数分三态"的写法曾试过，已按领队裁定整体回退）；
	//   箭头降级与语义保留都归 `cleanLine` 管，这里只管记号与标点空白。
	//   ⚠️ 下面两条 `，因此` 规则是**标点规整**（收拾作者手写的重复或错位），**不是**把箭头换成"因此"。

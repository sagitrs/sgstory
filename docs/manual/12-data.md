...
> ★**行效果（`gives`／`sets`／`yields`）今天不在 `links[]` 里**（`#1406`）：链接搬进数据面后，
> "点它会发生什么"**无处声明** ⇒ 若从规则行搬过去会**静默丢弃**（✗ 无判据会红 ✗）。
> ⇒ 今天请用**混合形态**：链接进 `links[]`，**带效果的行留在 `data/rules.json`**（正文保留对应 `<<rules>>`／`<<rulelist>>` 宏）✓；
> ★新票落地后，这一维才会真正落在数据面（届时本表补一行）。
...
## 12.1b 战斗段怎么写（★**收尾后的去向**：`<<fightpanel "池" N [won|lost] ["去向段名"]>>`）[锚]

```
· 第 3 参＝打满 N 回合即收尾 ｜ 第 4 参＝作者显式给的结果（`won`/`lost`；缺省＝不判胜负）
· 第 5 参＝**收尾那一击之后去哪一段**（缺省 ⇒ 与今天逐字相同 ✓）
  ⇒ 写法例：`<<fightpanel "野狼·林口" 3 won "林口·战后">>` ⇒ 打满 3 回合后**直接被带去**「林口·战后」
★分叉仍归你：到「林口·战后」那段再用条件行分叉（`fight:野狼·林口.won` ⇒ 胜／`.lost` ⇒ 败）
★死透优先：若那一击把玩家打死 ⇒ **仍去「结局 死亡」**（去向不覆盖死亡结局 ✓）
★去向段名写错 ⇒ **当场 fail-loud 点名**（页面会显示"段落不存在：…（战斗池：…）"）✗ 不会静默留在原地

★★**实现真相**：**同一池名，一次存档只收尾一次** ⇒ 若要「再打一次同样的敌人」，**请换池名**
  （如 `野狼·林口` ／ `野狼·夜袭`）—— ✗ 不然第二次的胜负**不会进表**（打完却按旧结果分叉 ✗）
```

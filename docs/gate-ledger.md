...
**严格行为化率（有自证）：12/44 = 27.3%** ｜ **有断言但缺自证：15**（＝下方工作清单）｜ 仅登记：7
...
| `audit:sel` | audit 开关 | 仅登记 | — | — | --sel 只是 --nosl＋--gear 的便捷别名；链上跑的是更具体的两个 flag，故没有单独的 --sel --check |
| `audit:sitedisc` | audit 开关 | 行为化（缺自证） | — | ✅ |  |
| `audit:social` | audit 开关 | 行为化（缺自证） | — | ✅ |  |
| `audit:starbudget` | audit 开关 | 行为化（缺自证） | — | ✅ |  |
| `audit:systems` | audit 开关 | 仅登记 | — | ✅ | 机制×锚句报表；机制可感知性由 --investment 的 G1/G5/G6 段承担 |
| `audit:text` | audit 开关 | 仅登记 | — | ✅ | 文本载荷报表；密度阈值已由 --craft 门 ratchet |
...
| `scripts/report-ledger-freshness.mjs` | 报告脚本 | 行为化 | ✅ | — | 需要网络与 token（GITHUB_TOKEN）——不塞进 npm test 主链路；由人工/定时跑 npm run report:freshness:check |

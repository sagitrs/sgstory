...
//
//注意：为什么浏览器侧的"写"不是 fs：静态页**不能落盘** → 这里的"写"＝把文本**收集起来**，
 ...
 *注意：这是本片"能假"的那条：把 `data/contract.json` 从入参里去掉 → 必抛；把断言删掉 → 测试红。 */
export const assertExportComplete = ({ slug, written = [] } = {}) => {
	//注意：`#1047` K6 复核：本件**不得**再定义与 core 同名的件（`core/ruleRows.mjs` 已导出 `norm`）

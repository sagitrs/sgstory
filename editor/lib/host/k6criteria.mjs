...
//注意：**为什么不是 core**：第一版按"看起来纯"放 core → 被**真 import** 否掉（用 `join`／`dirname`／
// `fileURLToPath` → 不纯）；而 **K6 ③ 的口径**正是"core 不许碰宿主" → 是**适用面**（＝判据的理由）
// 要求它们住 host。
//注意：**按实测坐标整块搬** —— 本文件块内**夹着**自证夹具（`:44–:45`）与 `mjsFiles`（`:47–:58`）→
...
export const primitiveWriteProblems = (files, hostDir = 'editor/lib/host') => {
	...
	for (const [path, text] of files) {
		...
		//注意：**必须遮字符串**：观测对象是"**代码里的调用**"，而字符串里写 `writeFileSync(...)` 只是**片段**
 ...
 *注意：**不进表**的东西（进了就会误报一片 → 门会被关掉）：`JSON`/`Math`/`Promise`/`Object`/`Array` 等**通用**全局；
 * 还有 **`console`** —— 它两宿主都有、core 里**可用**；"core 不该拿它当输出通道"是**另一条判据**
 *（"诊断是数据"）→ **别把两个问题绑在一起**。
 *注意：**必须同时覆盖 `globalThis.<名>`**（只匹配裸标识符会漏它 —— 与 `#806` 漏动态 import 同族；
...
export const coreHostGlobalProblems = (files) => {
	...
	//注意：只看**用法**（`名(` 调用 ／ `名.` 属性访问），不看"出现" —— 因为 core 会**合法地提及**它们：
 ...
 *注意：**显式边界**：**运行期拼路径的删除不在覆盖内**（`const p = join('stories', …); rmSync(p)` → 本判据**看不见**）
 * —— 别假装覆盖（与本门"引号未闭合 → 明记边界"同形）；要收它得做数据流分析（成本高、收益低）。 */
export const deleteStoryProblems = (files) => {
	const out = [];
	for (const [path, text] of files) {
		//注意：遮蔽口径**与 L1 相反**（今天第三个口径）：L1′ 的观测对象是"**调用参数里的字符串**"

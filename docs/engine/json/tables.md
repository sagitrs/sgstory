...
| ★**面内/面外（判据范围）** | 本门**只判「改制面」**（该故事有 `data/passages.json` **且**该段在 `data` 里有段对象） | **义务不可追溯**：旧形态作者今天**没有**这条义务（旧承载者已随旧故事删除）=> 拿新门追旧故事＝**跨仓破坏** ✗。与 P1–P4 **同尺**：**「不在面内 ≠ 下架/欠账」** ✓ |
| ★**并存（面内）** | `ending:{key,kind}` ＋ 正文手写 `<<ending>>` **同时存在** => **红** | 产物会出**两张出口卡** ✗ => 违「**声明处唯一**」红线（删正文那行 ✓）。天然能假：产物 `<<ending` 计数 **≠ 1** ✓ |
...
### 11.4 **`links[]` 字段面**（`#1406`）—— 白名单 ＋ **行效果**（`gives`／`sets`／`yields`）

> 契约（✗ 不是实现说明）：`data/passages.json` 的 `links[]` 里**只许**下列字段；**白名单外 ⇒ 编译期点名**
> （✗ 不许静默丢弃 —— 实测病灶：`gives` 曾被静默丢弃而 `problems=0` ✗）。

| 字段 | 型 | 含义 |
|---|---|---|
| `label`／`to` | `string` | 链接文案／目标段（缺件由 P1–P4 点名） |
| `id` | `string` | 行 id（`prereq` 与判据引用它） |
| `prio`／`prereq` | `number`／`string[]` | 顺序维（与规则行同义） |
| `cond` | `{req?,any?,exclude?}` | 条件维（**原样**搬进规则行 ⇒ 求值走 `Sg.rules.matches` **一处权威**） |
| `args` | `object` | 传值面（跳转携带的入参，见 §11.2） |
| `slot` | `string` | 落位（内联；带 `slot` 的行**不进**段尾块 —— 互斥落位 ✓） |
| ★**`gives`** | `string[]` | **行效果·物品面**（点它 ⇒ 授予物品） |
| ★**`sets`** | `string[]` | **行效果·状态键面** |
| ★**`yields`** | `string[]` | **行效果·笔记面** |

**口径：**
```
· ★★**行效果的施加时刻＝「点击那一刻」**（`#1408` 裁定，承 `#1406` ①）—— 与 `args`／跳转**同刻** ✓
  ⇒ 效果**随链接走**：编译期编进 `<a data-sg-effects="…">`（唯一权威＝`linkHtml`），
    由 `#1350` 片 5 那处**点击处理器**施加（与 `Sg.inargs.carry` 同一处）✓
  ✗ **不进规则行**：规则行语义＝「该作用域**渲染时**即施加」⇒ 读者**什么都没点就拿到钥匙** ✗
    （T 两态实测：`gives` 挂非入口段链接 ⇒ **一进那段（未点任何链接）`inv` 里就有了** ✗）
· ★**施加器仍是同一个**（`Sg.rules.applyYields/applyGrants/applySets` ⇒ ✗ 不造第二套 → 幂等／写点单一照旧 ✓）
· ★**为什么必须有**（`#1406` 实测病灶）：链接搬进 `links[]` 后**没地方放行效果** ⇒ `gives` 被**静默丢弃**、
  玩家拿不到钥匙、门永远打不开，而**没有任何判据会红** ✗ ——「静默丢维」的具体实例 ✓
```

### 11.5 **可达性**（`#1401`）—— 段必须能从 `entry` 走到（引擎侧**静态图机制**）

> 契约（✗ 不是实现说明）：判据面里此前**没有"可达性"这一维** ⇒ "能 build、能渲染、就是**没人走得到**" ✗。

| 项 | 约定 | 为什么 |
|---|---|---|
| 判据 | `unreachablePassages({data, entry, rules})`（**纯函数** ⇒ 能假） | 静态图（✗ 不做运行时巡游）✓ |
| **出边面** | ★**两处并集**：① `passages.json` 的 `links[].to` ② **规则行** `text` 里的 `[[label\|to]]` | 出边**分散在两地** ⇒ 只算一处**不是漏报就是误报**：实测"只算 `passages.json`"⇒ **误报**「废哨站」（它的入边在 `rules.json` 的行里）✓ |
| 根 | `00-story.json` 的 `entry` | `entry` 自身**不算**死段 ✓ |
| 传递 | 沿出边做可达闭包（含自环／回边） | 段→段→… 都算 ✓ |
| 输出 | 不可达段**点名** ＋ **修法**（加链接／删段／设为 `entry`） | 与 P1–P4 同体例（点名 ＋ 可执行的那一步 ✓） |
| ★**范围**（Operator 纪律） | 本机制属**引擎**（fixture 驱动自证）；**"对真故事跑"＝ books CI 的门** | 引擎主干**不判故事策略**（✗ 引擎不跑真故事）✓ |

**★ 为什么必须成机制**（作者实测）：`build` **rc=0**、用例**全绿**（没人走它，自然不红）⇒
"**零命中 ≠ 已覆盖**"的具体实例 —— 没被任何路径走到的段，在今天的判据面上**完全不可见** ✓
### 11.6 **双渲染宏**（`#1412`）—— ✗ 不许"手写渲染宏 ＋ `links[]` 非空"并存

> 契约（✗ 不是实现说明）：段数据的 `links[]`（**不带 `slot`** 的那些）会在**编译期注入段尾块**
> （`<<rules>>`／`<<rulelist>>`）⇒ 若**正文已手写**同一个宏 ⇒ 产物里**渲染两遍**（读者看到两组链接）✗。

| 项 | 约定 | 为什么 |
|---|---|---|
| **面内不判** | 只判**改制面**（该故事有 `data/passages.json` **且**该段在 `data` 里有段对象）；否则 ✗ 不判 | 与 `endingProblems` **同尺**：**义务不可追溯**（旧形态作者没有这条义务）✓ |
| **点名条件** | 面内段**同时**有散文手写 `<<rules>>`／`<<rulelist>>` **与** 会**注入段尾块**的 `links[]` ⇒ **红** | "看起来能跑、其实重复渲染" ✗（**无法**从"能 build／能渲染"看出）✓ |
| **例外** | **带 `slot` 的 links** ＝ **内联落位**（✗ 不注入段尾块）⇒ 与手写宏**不冲突** ✓ | 内联 ⊕ 段尾 ＝ **互斥落位**（`#1350` 片 5 同口径）✓ |
| **修法** | ① **删正文那一行宏**（链接由编译期注入 ✓）或 ② 给这些链接加 `slot` 走**内联落位** ✓ | 与 P1–P4 同体例（点名 ＋ **可执行的那一步**）✓ |
| ★**能假** | 该段**渲染宏计数 ≠ 1**（产物侧）✓ | 与 `#1399`「并存 ⇒ 红」**同族** ✓ |

### 11.7 **战斗结果维**（`#1413`）—— `fight:<池名>.won` / `.lost` / `.done`

> 契约（✗ 不是实现说明）：新格式下作者**能起战斗**（`<<fightbegin>>`／`<<fightlog>>`／`<<fightpanel>>`），
> 但**"赢／输"无处声明** ✗ —— 旧故事只能在**散文里写计算**（`<<if $pc.hp gt 0>>` ＋ `round` 比较 ＋ 手置 `done`）。

| 项 | 约定 | 为什么 |
|---|---|---|
| **前缀族** | `fight:<池名>.<字段>`（与 **`chk:` 同构**：前缀族＝引擎 schema／池名＝故事数据） | ✗ 不新造第二套；族集合由 `test/readkey-family.mjs` **成对断言**（引擎 ≡ core 镜像）✓ |
| **字段** | `won`／`lost`／`done`（＋ `round`／`pcHp` 事实读数） | 条件行可写 `cond:{req:['fight:雾影.won']}` ⇒ **声明态**，与"条件行只有声明态一维"自洽 ✓ |
| **写入点（唯一）** | `<<fightend "<池名>" [won\|lost]>>` ⇒ `Game.Combat.settle(pc, pool, want)` ⇒ 落 `State.variables.fights[池名]` | ✗ 不新开写路（与 `checks` 同规）；`<<fightbegin>>` 读 `pc.ev.fight.done` 判"新一场"（既有字段 ✓） |
| ★**结果由谁定** | **作者显式给**（`won`／`lost`）｜不传 ⇒ 只落事实（`won`/`lost` 都为 false ⇒ 分叉都不出） | ✗ **引擎不判"打够几回合算赢"** —— 那是**故事策略**（引擎写死阈值＝把"哪个故事怎么算赢"塞进引擎 ✗） |
| ★**未收尾就引用** | ⇒ **fail-loud 点名**（照 `chk:` 那条"本次尚未检定"先例） | ✗ 不静默当 `false`（那会变成**恒假的假面** ✗） |
| ★**幂等** | 已 `done` 的池**不被覆盖**（✗ 不覆盖首次结果） | 重渲染/重复调用安全 ✓ |

**★ 收尾时机（`#1426`，本节的另一半）**：`<<fightpanel "<池名>" N [won\|lost]>>`
```
· 第 3 参＝**打满 N 回合即收尾**（**机械事实**：回合数 ✓）｜第 4 参＝**作者显式给的结果**（缺省＝**不判** ⇒ 两侧分叉都不出）
· ★引擎仍**不判胜负**（✗ 那是故事策略）｜✗ 不新开写路（复用 `Game.Combat.settle` ✓）｜✗ 不破零表达式（作者写的仍是具名动作 ＋ 字面量参数 ✓）
· ★兼容：两参**可选** ⇒ 不传 ⇒ 行为与今天**逐字相同** ✓
· ★★**实现要点（实测）**：判定必须落在 **link 体**（＝**点击时**执行）——
  因为 SugarCube 对 `<<goto 同段>>` **不重渲染**（实测：`:passagerender` 计数 1→1）
  ⇒ 写在渲染期的判定在"打满"那一刻**永不执行** ✗
```

**★ 收尾后的去向（`#1432`，作者裁定＝③「收尾后自动跳转」）**：`<<fightpanel "<池名>" N [won\|lost] ["<去向段名>"]>>`
```
· 第 5 参＝**收尾那一击之后带读者去的段名**（单向）｜**缺省 ⇒ 与今天逐字相同** ✓（既有故事/夹具零影响 ✓）
· ★**分叉仍归作者**：去向是**单一段名** ⇒ 到那段再由条件行分叉（`fight:<池>.won` ⇒ 胜／`.lost` ⇒ 败）
  ⇒ **去向只有一处权威**（✗ 引擎不在第 5 参上做胜负二分 ⇒ 两处声明必漂移 ✗）
· ★**死透优先**：去向跳转排在 `$pc.hp gt 0` **之内** ⇒ 死透那一击仍去「结局 死亡」（✗ 不被覆盖）✓
· ★**段名取不到 ⇒ fail-loud 点名**（报**段名 ＋ 池名**；SugarCube 会把宏内错误**渲染成页面可见文本** ⇒ 作者当场看得见）✓
  ✗ 不静默跳成荒地；✗ 也不当作「没写去向」而静默留在原段
· ★**为什么不做成「收尾后不渲动作」**：那条只能读**持久**的 `fights[池].done` ⇒ ① `settle` **幂等** ⇒
  同一池**再打一场，结果被丢掉**（静默空转 ✗）② 同一池名**跨章复用** ⇒ **无敌可打**（面板空 ✗）
  ⇒ ③ 把读者**带出这一段**，天然避开两者（面板自然消失，✗ 不留可点的空转按钮）✓
· ★★**实现要点（实测）**：跳转落在 **link 体**（＝**点击那一刻**，与 `settle` 同一处）——
  · `<<goto>>` 宏＝`setTimeout(() => Engine.play(…), Engine.DOM_DELAY)` ⇒ **异步**（默认 40ms ⇒ 观感即「立刻」）
    ⇒ 这才是**渲染期导航的正路** ✓（而**直接**调 `Engine.play(dest)` 在渲染期**被引擎拒** ✗）
  · ⇒ 判据读「跳没跳」要**等一拍**（✗ 点完立刻读 ⇒ 假红；本仓既有先例：`<<pickPreset>>` 用 `Engine.play` 于**点击**路径）
```

**★ 实现真相（契约，`#1432` 作者指出）**：**同一池名，一次存档只收尾一次**（`settle` 幂等，✗ 已 `done` 不被覆盖）
⇒ 故事若要「**再打一次同样的敌人**」⇒ **必须换池名**（如 `野狼·林口`／`野狼·夜袭`）
  ✗ 否则第二次的胜负**不会进表**（而场上照打 ⇒ 读者会看到「打完却按旧结果分叉」✗）

**★ 落法两坑（实现侧，实测）**：① `$args` 是**宏上下文**量 ⇒ ✗ 在传给函数的函数体内取不到（恒空）
② SugarCube 的 `<<run>>` **只吃单行表达式** ⇒ 多行函数体会被截断（`Unexpected end of input`）
⇒ 口径：**宏体顶层取局部量** ＋ **逻辑放 `Game.Combat.settle`（引擎侧一处纯函数，可由 node 驱动 ✓）**。
★ 另一处同族教训：`hp` **非有限数时不许静默判输**（`!(undefined > 0)` 恒真 ⇒ 会把"没收尾"误判成"输了" ✗）。

...
> **状态**：v1.7（**M1 骨架 + M2–M4 文案 + M5 终局 + M6a–f 打磨 + M5c/M5d 花田 + M5e 换哨口径已完成**）
...
`宴会·过去` → `观星者`（`观星者·星` / `观星者·夜` / `观星者·图` → **完整星图**）/ `当时的女巫`（`当时的女巫·换` → **好哨**，前提：已取图）/ `老巫女`（`老巫女·跑` → 过去知；`告知与告别` → **卷轴**，前提：已取图）/ `喂花` / `老妇人`
...
| `failure_cause` | 那一夜发不动＝烧的是它自己的力气，而它攒得还不够 | 过去·老巫女（穿越回来的那位）/ 现在·书房 / 现在·交付 |

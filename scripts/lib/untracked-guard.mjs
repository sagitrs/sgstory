...
//注意：**不采用"新增 `--check` 档"**（票面 §三 甲）：三门都是"**裸跑即判定**"形态（`npm test` 里裸调）
 ...
 *
 *注意：**为什么必须有这一格**（本片实测踩到）：本判定读的是 `git ls-files --others`（**工作树实况**）
 ...
 *注意：**边界（不夸大）**：本谓词只挡"**仓内已知的临时夹具命名**" —— 若将来有段用别的名字造临时件，
...
/** 未跟踪豁免的行首标记。注意：理由与票号**少任一项 → 不生效**（与 `deauth-exempt` 同口径）。 */
 ...
 *
 *注意：「有理由」的判据是「**除票号外还有实质文字**」 —— 不是"长度≥N"（本件自证实测：

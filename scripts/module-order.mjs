...
//注意：因此本模块**不**做「文本里出现即算引用」的推断式 lint：那种推断会产出假阳性
 ...
 *注意：必须用**路径**兜底（不是"只在 `MODULES` 里查"）：引擎件若漏登记 `MODULES`，只查后者会把它**当成故事件**
 ...
 *注意：**不要求**故事件**进 ORDER**（`#893` 起故事件改由**自己的清单**登记）——
...
export const readModules = () => {
	...
	const out = {};
	// `#1267` 尾件①：`allSourceFiles()` 含**符号名**（stories/…）→ 必须过 `absPath`（仓内恒等；
...
export const STORY_SYMBOLS = [
	...
	//注意：残留的过渡味道（记在案，不假装没有）：机制与数据**共用同一个命名空间**（引擎把 `resolve` 挂在故事提供的 `Game.Checks` 上）。
...
//注意：本谓词是**唯一一份定义**（另一处 import 它 —— 两份逐字相同的副本会漂，已有自证格钉住）。
 ...
 *
 *注意：为什么要有这一格：`checkRegistration` 只遍历**默认源面**（twee ＋ md）→ `stories/<x>/*.json` **根本不在它的输入里**
 * → ∴ 本格**自己枚举 json 面**（这是**新谓词面**"json 住在哪" —— 不是第二份**源集合**枚举，与"同一权威"不冲突）
 *注意：**将来有人加第五类角色 → 本格会红一次** → **那正是设计**：
 ...
 *
 *注意：两个被读数推翻的版本（都留在这里，因为"为什么不是那样"才是关键）：
 ...
 * → 那会**改既有故事的产物**。
 *注意：**读数更正**（`#895` 复核发现非阻塞项）：这里原来写的是"**三个**故事的清单序都不等于 `ORDER` 序" ——
 ...
 *
 *注意：为什么不是"放宽 `ORDER` 守卫"：`test/layering.mjs` 明写"新增文件不得靠文件名前缀**自动**获得顺序"
 ...
 *
 *注意：`#893`：故事自己的件**按清单顺序**（不再用 `ORDER` 过滤 —— 否则新建故事不碰代码就排不进顺序）。
 */

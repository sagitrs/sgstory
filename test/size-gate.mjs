...
// `#1267` 尾件②（复核裁定）：**外根且该根下没有自己的基线 → 显式「未判」**（rc=0 ＋ 明说）。
// 理由：本门量的是**该根的产物**，而基线是**该根的预算** —— 没预算就无从判「超没超」（原实现
// 把「没基线」当成「每一项都缺失」→ 一律红，那是把「未判」误报成「失败」）。
//   不用白名单（那正是本线一直在清的东西）；**能假格**：该根下有基线且体积回归 → 仍红。
if (!WRITABLE) {
	console.log('  #1267 外根无自己的体积基线 ⇒ 本门**未判**（该根预算未建立；如需建立请在该根上层放基线）');
	process.exit(0);
}

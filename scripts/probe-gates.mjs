 ...
 *注意：原来一律报「变异前就红 → 这次"红"不是变异造成的」→ 读的人会去查**判据**，而真因是**树上产物陈旧/缺失**
 ...
 *
 *注意：**本函数只判"入口件的直接 import"**（**已经过一轮纠偏**，`#1019` 复核席点名）：
...
const selfcheck = () => {
	...
	//注意：**不做传递展开**（`#1019` 实测的教训）：第一版按"件内出现的 `.mjs` 路径"向下扫 2 层 → 把
...
const probeOne = (p) => {
	...
	//注意：报的是**病因名**：判据没坏，是**读数不成立**（`#1018` 实测那条假读数就是它）。
	...
	try {
		...
		//（注意：不能用 `pre` 表达这一处：`pre` 的语义是"**变异之前**"，现有十五探针依赖它，不改。）
...
const checkStructure = () => {
	...
	//注意：它同时咬住「`cmd` 指错件」与「重复键合并把两条并一条」（合并 → `cmd` 必然 ≠ `id`）——

...
//
//注意：为什么不做成"逐故事跑全套"：实测 K5／K6 是**全局**面、K3 半是引擎级 → 照字面做不到
//（`#984` 的接口口径：**全局面每轮一次 ＋ 故事作用域面逐故事**）。
//注意：**单跑前先跑构建相**：① 逐故事面里有需要构建产物的段 → 先 `node build.mjs`；② 全局面 **K5**（台账
...
if (isMain) {
	...
	if (argv.includes('--selftest')) {
		const cases = [
			...
			// 要断言的是「**真故事都在**」（⊇），不是「只有它们」（＝）。
			//注意：`#1004` B2b：这一格原来**写死三个 slug**（`hollow-cave`／`minimal-demo`／`mist-forest`）→ 故事一删
			...
			//注意：三个数不一样是**对的**：表里 global 有 **5** 条（K2/K3/K4/K5/K6），但 **K3 是 heavy**
	...
	//注意：**取不到输入就不许判过**（与 `commands.mjs` 同口径）：发现到 0 个故事时逐故事面会**整批消失**，
	...
	//注意：**唯一裁决**（`#989`）：末行的 `✔/ `、`x/y`、清单、**退出码**全从这一处长出来

...
⚠️ **必须显式给空，不许省略**：空表＝合法数据集；**不给空表就等于借用别的故事的判据**（`#602` 的旧洞）。
...
`night-ferry`（P4 的产物）实测含以下件（⚠️ **与清单的 `gates` 数组是两回事**：清单 `gates: []` ＝ 不声明"可执行门"；`gates/` 目录里的**见证/基线夹具**照常存在并被 `--equiv` 等消费——别把"目录非空"读成"清单漏登记"）：

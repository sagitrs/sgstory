...
//注意：jsdom 收场纪律：`boot()` 各自 close ＋ 末尾显式 `process.exit(rc)`。
...
//注意：**本件当前仍红**（`#1004` B2b 复核席登记，不是"改坏了"注意：）：
// 面夹具**声明了**规则面（`17-rules.twee` 两行：`ev.听雾`／`ev.换哨`），但**没有任何段落消费它**
//（`grep -n 'rulelist\|<<rules>>' stories/face-fixture/*.twee` → 0 命中；本件用的段落 `洞穴` 也不渲染规则行）
//注意：**`#1114` 片 2b-1 更正（2026-09-21）：上面两行断言已过时** —— 现实况
// `grep -n 'rulelist\|<<rules' stories/face-fixture/10-fixture.twee` → **12 处消费**
//（`rulelist` 7 ＋ `rules` 5 其中 4 处 `<<rules "scope">>` 由本片新加：`森林边缘`／`门厅`／`塔外花田`／`老巫女`）；
// 而**本件的对象早在甲案就已就位**（`洞穴` 段的 `<<rulelist "洞穴#火光">>` 消费 `洞穴.火光.有火把`）。
// 术语“无段落消费”与“本件仍红”**均已不成立**，保留原文仅作留痕。
// → 本件的核心判据「**改一个事件的字段 → 预览随之变化**」当前**没有对象**。
// → 处置二选一（归发起者／开发席）：**（甲）**夹具加一个消费规则面的段落（如 `<<rulelist>>` —— 声明了面就该有消费者）；
// **（乙）**按"夹具接不住 → 退役＋声明"处理本件的真数据半边。
//注意：两案我都能执行；**本轮不改它的判据** —— 先把缺口报上去（免得替夹具侧决定它该接哪个面）。
const EVENT = '洞穴.火光.有火把';   // `#1004` B2b：**按实测改回旧 id** —— 夹具按裁定 (甲) 沿用了它，且 `洞穴` 段有 `<<rulelist "洞穴#火光">>` 真的消费它（我中途按"旧 id 已消失"的**假设**改成 `ev.听雾` → 那是错的）
const MARKER = '【预览探针】';
//注意：**渲染面：三个假设全被推翻，且两条实测互相矛盾 **（如实记，**不写结论**）：
...
// →注意：**冲突**：同一件事（链的编辑追加 `MARKER+ABC`）在链的读数里 span **不含 ABC**、在直接实验里**含**
 ...
 *注意：复核席两次判过这里：**两侧各自独立 boot 才算在比两条路** —— 同一个 `w` 读两遍是**恒等式**。
...
/**注意：**能假**的"差异段紧致度"读数 —— 替换掉原来那句**构造上恒真**的"重建相等"：
...
/** 全新 boot ＋ **在页面里**钉状态（注意：跨 realm 的对象喂给 `State.variables` 会 clone 失败 → 必须 `eval`）＋ 取预览。 */
const preview = async ({ story = null, gear = [], keepWin = false } = {}) => {
	//注意：两处实测坑（都当场抓的）：① 探针 slug 没有 `stories/<slug>/00-story.json` → 必须显式 `entry` 绕开；
...
const selftest = () => {
	...
	//注意：这里原本是 `t('自证自身能红…', 1 === 2? false: true)` —— **恒真** → 不是断言
	...
	// 合成输入 —— **两段不相邻差异** → "恰好一段"那条**必须为假**（区间法只能过一段）。
	{
		...
		//注意：并记一条**反例存档**："两串差异是一段连续区间"这个说法**本身不可假** ——
...
try {
	...
	//注意：复核席第五轮: 这里原是 `rebuilt === T3 &&!d.same" —— `rebuilt` 正是按**最长公共前后缀**
	...
	//注意：我第一版把"重排"写成 `JSON.parse(JSON.stringify(x, null, 6))` → **缩进被解析丢掉了**
	...
	// (ii) **每一步各自报出它启的产物 sha 且相等** —— 不许"我传了参数就算同一份"
	{
		...
		//注意：`savePackage` **不收 io** —— 它用 `collectIo()` 纯收集 → **返回字节**，落盘由宿主做
		...
		//注意：复核席裁定 (b)：**页内侧也必须读磁盘** —— 只换 `rules.json` 的话，
		...
		//注意：用**同一 slug** → 输出里若带 slug 也两侧一致（不必假设"规则文本与 slug 无关"）
		...
		//注意：原写 `--story-out=${join(PROBE_DIR,'index.html')}`（**绝对**）→ `build.mjs` 的 `join(ROOT, STORY_OUT)` 会拼成
		...
		//注意：**归属**在**页面字节**上量（含 `〈链〉`，见下一条）；**紧致度**按**渲染**里的标记量：
		...
		//注意：这是**观察打印**（期望值未知 → 不写成断言 —— 不许把"看着像"写成判据）。
	...
	//注意：复核席第 4 次拦下这里：字面 `true` 的"断言"＝没断言；`sha(f)===probeSha`（同源）＝恒真
	// → 本次三条都补齐：**实例不同** ／**两侧产物 sha 相同且 ≠ 真 dist** ／**注入两侧都被消费**。
	{
		...
		try {
			...
			//注意：我上一版写成 `probeSha === sha(probeUrl)` → **还是自比自**（复核席追到）。

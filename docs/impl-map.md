...
> **状态**：v2.0（**M1 骨架 + M2–M4 文案 + M5 终局 + M6a–f 打磨 + M5c–M5g 口径 + M7 v17 全案（去神秘化 / 传说层 / 好感合一 / 封印战）已完成**）
...
`守林人` → **`守林人·封印`（"你把它按下去，我念" → `keeper.state=seal`）** / `门厅`（坏哨）→ `书房`（日记）→ `温室`（空盆）→ `工坊`（龙鳞护臂）→ `天文台`（观星者的书 / 碎镜片 / 星账）→ `顶楼`；花在 **`塔门` → `塔外花田`**（自由选择点）
`塔门` → `半途的林子` / `结局 半途`；`顶楼` → `结局 新任守林人` / `结局 焚塔者` / `结局 讨伐`

### 三章 · 坐标
`地下宴会厅`（现在：只有龙；`龙·巢边` 星名页 / 攻击 / **`叫醒它`**）→ 翻转 → `宴会·过去`（hub）
...
| `test/scenarios.mjs` | **23 条路线**（金路径 + 全部结局 + 设定集 + 龙巢边） | 重写 |
| `test/smoke.mjs` | 启动 → 快速车卡 → 酒馆 → 森林 → 洞穴 + 侧栏/存档/物品栏 | 重写 |
| `test/coverage.mjs` | 渲染/交互覆盖率 ratchet | **收紧为 5 门**（基线：渲染 **78** / 交互 **83**） |

**机检六门**：`--truth` `--canon` `--echoes` `--choices` `--systems` `--text`。

**canon 门（M1b）**：`docs/lore-canon.md` §10「已裁剪设定」是唯一黑名单来源。门做两件事——① **行覆盖**：§10 每一行必须被 `CANON_ROWS` 认领（新增裁剪行不认领即红）；② **词扫描**：认领行禁词不得出现在 shipped 文本（正文 + 数据表字符串；`/% %/`、JS 行注释、CSS 块注释不计），否定句（如「没有亡灵」「不集齐开锁」）允许。另含定向检查：`守林人*` 段落不得出现「她」。当前：§10 行 **70** · 认领 **71** 条 · 禁词 **121** 个 · 命中 0。

**§9 双读断言（M6a）**：只扫正文（不扫 `[script]`/`[stylesheet]`），禁 6 条断言——`长生` · `同一个人` · `初代巫女` · `初代` · `穿越` · `晚年`。命中即红并指出段落。当前命中 0；注入测试已验证（`设定集·三律` 注入「长生 / 同一个人」→ 门红并报段落）。

**当前规模**：`npm test` 全绿（**23 条路线** · 内容段 68 · 渲染 78 · 交互 83 · 正文 ≈1.33 万字）；`npm run soak`（游走 20+20 局 + **18 位点**双支）0 违法，**已接回 CI 独立 job**（`deploy` 依赖 `[test, soak]`）。
...
5. `test/scenarios.mjs` 路线 → 全绿 ✅（现 **23 条**）
6. `npm run soak` → 0 违法 ✅

...
**严格行为化率（有自证）：73/117 = 62.4%** ｜ **有断言但缺自证：43**（＝下方工作清单）｜ 仅登记：0
**探针（直接读数 ✓，不是"文件在不在"那种代理 ✗）：`✅` 32 项 ｜ `—` 未探 84 项（**上限 117** ✓ 超过即红 ✗；**调高它**是一次显式手改 ⇒ 靠评审拦 ✗，机器拦不住“手改上限”本身 ✓ —— 边界记在票 #908 内 ✗）｜ `✗` 不咬 1 项（**>0 即红** ✓）** —— 档位／清单：`node scripts/probe-gates.mjs --probe=fast` ✓（⑲：本轮覆盖到哪一档写在这行里 ✓）
...
| `test/notes-absence.mjs` | 测试脚本 | 行为化（缺自证） | — | ✅ | ✅ |  |

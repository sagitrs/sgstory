...
`run.sh` 做三件事：**清三层 ⇒ build ⇒ 跑用例**，退出码＝用例执行器的退出码。它存在的理由不是省事，
...
为什么非得清：这些**都被 `.gitignore` 忽略** ⇒ `git status` 看不出它们还在 ⇒ 忘了清时 build 会「按件在」
**复用旧产物** ⇒ 你在**两个引擎态下会读到同一个结论**（那时它判的不是修复，而是旧故事／旧引擎）。

...
// ③（**提示，不作判据**）**草稿基数**：跑完打印 `build/generated/` 里新增的 `.lint-run-*` 数 ——注意：共享目录**无法归因**
...
//注意：**本件自己准备环境**：旧落点可能因为**先前**跑过别的工具而已经存在（例如早于本修法时跑的
...
//注意：**只判「本次新出现的」** —— 全局列举式断言**天生竞态**：兄弟段（`test-plan` 的 `test-lint-story-mjs`
...
//注意：**只作提示，不作判据**（本片合入后 CI 实证的第二次同类坑）：`build/generated/` 是**共享目录**，

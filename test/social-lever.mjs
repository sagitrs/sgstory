...
//
// ⚠️ 与退役件的**唯一取值方式差异**（**判据强度不变** ✗）：`advWhy` 的期望文案**不写死** ✗ ——
//   从夹具数据 `tables.json` 读该筹码的 `note` 再断言 `advWhy` **包含**它 ✓
//   （旧件写死「先人」是**旧样本的内容**，换样本就得跟着换值；写死新值会把下一轮"换样本"再罚一次 ✗）。
//
// ⚠️ **驱动面（如实声明，见件尾"覆盖面差异"）** ✗：退役件点 DOM 上的 `a.soc-opt` 走**点击处理器**；
 ...
 *  ⚠️ **每一步都要重开** ✗ —— 这正是引擎点击处理器的做法
 *  （`src/80-script.twee` `click.sgSocial`：每次先写 `pc.ev.soc = {id, how}` 再 `Engine.play` ✓）
 *  ⇒ 本件按同一时序驱动 ✓。
 *  ⚠️ `pc.soc` 要**全形** `{att,tries,read}`（引擎 `settle()` 自己初始化的就是这三键 ✓）——
...
// ⚠️ **不要拿 `done()` 当「诉求完成了吗」** ✗ —— 夹具的 `done.req` 用了键形 `note:<id>`，

...
| `scripts/report-gate-ledger.mjs` | 报告脚本 | 行为化 | ✅ | ✅ | ✅ | **临时下架**：台账门：探针读数缺失时走降级路径（需与 E 表对齐）（until #1163（books 回填样本）） | 本文件自身的自检（台账不腐），已入 npm test |
...
| `test/audit-gates-run.mjs` | 测试脚本 | 行为化 | ✅ | — | ✅ | **临时下架**：审计跑器自证：需默认故事全绿（零故事下无默认故事）（until #1163（books 回填样本）） |  |
| `test/browser.mjs` | 测试脚本 | 行为化（缺自证） | — | — | — | — | 需真实 Chrome（npm run browser / soak）；CI 由 soak job 跑 |
| `test/chargen-apply.mjs` | 测试脚本 | 行为化（缺自证） | — | — | ✅ | **临时下架**：样本随 demo 下架（对象＝该用例本身，属引擎/工具面判据）（until #1163（books 回填样本）） |  |
| `test/chargen-macros.mjs` | 测试脚本 | 行为化（缺自证） | — | — | ✅ | **临时下架**：样本随 demo 下架（对象＝该用例本身，属引擎/工具面判据）（until #1163（books 回填样本）） |  |
| `test/choice-keys.mjs` | 测试脚本 | 行为化（缺自证） | — | — | ✅ | **临时下架**：样本随 demo 下架（对象＝该用例本身，属引擎/工具面判据）（until #1163（books 回填样本）） |  |
| `test/ci-triggers.mjs` | 测试脚本 | 行为化 | ✅ | ✅ | ✅ | — |  |
| `test/combat-adv.mjs` | 测试脚本 | 行为化（缺自证） | — | — | ✅ | **临时下架**：样本随 demo 下架（对象＝该用例本身，属引擎/工具面判据）（until #1163（books 回填样本）） |  |
| `test/comment-face-split.mjs` | 测试脚本 | 行为化 | ✅ | ✅ | ✅ | — |  |
| `test/comment-mask.mjs` | 测试脚本 | 行为化（缺自证） | — | — | ✅ | **临时下架**：样本随 demo 下架（对象＝该用例本身，属引擎/工具面判据）（until #1163（books 回填样本）） |  |
| `test/cond-keyform.mjs` | 测试脚本 | 行为化 | ✅ | — | ✅ | **临时下架**：对象＝通用机制（方言/契约版本/键形/词汇/工具不变量），样本随 demo 暂缺（until #1163（books 回填样本）） |  |
| `test/contract-compat.mjs` | 测试脚本 | 行为化（缺自证） | — | ✅ | ✅ | — |  |
| `test/contract-defaults.mjs` | 测试脚本 | 行为化（缺自证） | — | — | ✅ | **临时下架**：样本随 demo 下架（对象＝该用例本身，属引擎/工具面判据）（until #1163（books 回填样本）） |  |
| `test/contract-version.mjs` | 测试脚本 | 行为化（缺自证） | — | — | ✅ | **临时下架**：对象＝通用机制（方言/契约版本/键形/词汇/工具不变量），样本随 demo 暂缺（until #1163（books 回填样本）） |  |
| `test/core-story.mjs` | 测试脚本 | 行为化 | ✅ | — | ✅ | — |  |
| `test/coverage.mjs` | 测试脚本 | 行为化（缺自证） | — | — | ✅ | **临时下架**：样本随 demo 下架（对象＝该用例本身，属引擎/工具面判据）（until #1163（books 回填样本）） |  |
| `test/dialect.mjs` | 测试脚本 | 行为化（缺自证） | — | — | ✅ | **临时下架**：对象＝通用机制（方言/契约版本/键形/词汇/工具不变量），样本随 demo 暂缺（until #1163（books 回填样本）） |  |
| `test/docs-read-path.mjs` | 测试脚本 | 行为化 | ✅ | ✅ | ✅ | — |  |
| `test/equiv-scratch.mjs` | 测试脚本 | 行为化（缺自证） | — | — | ✅ | **临时下架**：对象＝通用机制（方言/契约版本/键形/词汇/工具不变量），样本随 demo 暂缺（until #1163（books 回填样本）） |  |
| `test/fight-seq.mjs` | 测试脚本 | 行为化（缺自证） | — | — | ✅ | **临时下架**：样本随 demo 下架（对象＝该用例本身，属引擎/工具面判据）（until #1163（books 回填样本）） |  |
| `test/focus-after-nav.mjs` | 测试脚本 | 行为化（缺自证） | — | — | ✅ | **临时下架**：样本随 demo 下架（对象＝该用例本身，属引擎/工具面判据）（until #1163（books 回填样本）） |  |
| `test/g3-evidence.mjs` | 测试脚本 | 行为化（缺自证） | — | — | ✅ | **临时下架**：样本随 demo 下架（对象＝该用例本身，属引擎/工具面判据）（until #1163（books 回填样本）） |  |
| `test/gate-discovery.mjs` | 测试脚本 | 行为化 | ✅ | — | ✅ | **临时下架**：样本随 demo 下架（对象＝该用例本身，属引擎/工具面判据）（until #1163（books 回填样本）） |  |
| `test/gen-needed.mjs` | 测试脚本 | 行为化（缺自证） | — | — | ✅ | **临时下架**：生成件清单：反向核需 ≥3 个真故事（until #1163（books 回填样本）） |  |
...
| `test/npm-entries-guard.mjs` | 测试脚本 | 行为化 | ✅ | — | ✅ | **临时下架**：npm 入口差集：取样脚本随 WebUI 下架 ⇒ 待改取样（until #1163（books 回填样本）） |  |
| `test/onetime-pickups.mjs` | 测试脚本 | 行为化（缺自证） | — | — | ✅ | **临时下架**：样本随 demo 下架（对象＝该用例本身，属引擎/工具面判据）（until #1163（books 回填样本）） |  |
| `test/passages-assemble.mjs` | 测试脚本 | 行为化 | ✅ | — | ✅ | — |  |
| `test/pc-base.mjs` | 测试脚本 | 行为化（缺自证） | — | — | ✅ | **临时下架**：样本随 demo 下架（对象＝该用例本身，属引擎/工具面判据）（until #1163（books 回填样本）） |  |
| `test/pc-defaults.mjs` | 测试脚本 | 行为化（缺自证） | — | — | ✅ | **临时下架**：样本随 demo 下架（对象＝该用例本身，属引擎/工具面判据）（until #1163（books 回填样本）） |  |
| `test/plan-needs.mjs` | 测试脚本 | 行为化 | ✅ | — | ✅ | **临时下架**：样本随 demo 下架（对象＝该用例本身，属引擎/工具面判据）（until #1163（books 回填样本）） |  |
| `test/premise-source.mjs` | 测试脚本 | 行为化 | ✅ | — | ✅ | — |  |
| `test/properties.mjs` | 测试脚本 | 行为化（缺自证） | — | — | ✅ | **临时下架**：样本随 demo 下架（对象＝该用例本身，属引擎/工具面判据）（until #1163（books 回填样本）） |  |
| `test/prose-vocabulary.mjs` | 测试脚本 | 行为化 | ✅ | — | ✅ | **临时下架**：对象＝通用机制（方言/契约版本/键形/词汇/工具不变量），样本随 demo 暂缺（until #1163（books 回填样本）） |  |
| `test/readkey-family.mjs` | 测试脚本 | 行为化（缺自证） | — | — | ✅ | **临时下架**：读键族抽取面：样本需真故事的键族分布（对象＝引擎面判据）（until #1163（books 回填样本）） |  |
| `test/render-all.mjs` | 测试脚本 | 行为化（缺自证） | — | — | ✅ | **临时下架**：样本随 demo 下架（对象＝该用例本身，属引擎/工具面判据）（until #1163（books 回填样本）） |  |
| `test/repo-shape.mjs` | 测试脚本 | 行为化 | ✅ | ✅ | ✅ | — |  |
| `test/reread.mjs` | 测试脚本 | 行为化（缺自证） | — | — | ✅ | **临时下架**：样本随 demo 下架（对象＝该用例本身，属引擎/工具面判据）（until #1163（books 回填样本）） |  |
| `test/roll-binding.mjs` | 测试脚本 | 行为化（缺自证） | — | — | ✅ | **临时下架**：样本随 demo 下架（对象＝该用例本身，属引擎/工具面判据）（until #1163（books 回填样本）） |  |
| `test/route-registry.mjs` | 测试脚本 | 行为化（缺自证） | — | ✅ | ✅ | — |  |
| `test/rules-claims.mjs` | 测试脚本 | 行为化 | ✅ | — | ✅ | — |  |
| `test/rules.mjs` | 测试脚本 | 行为化 | ✅ | — | ✅ | **临时下架**：样本随 demo 下架（对象＝该用例本身，属引擎/工具面判据）（until #1163（books 回填样本）） |  |
| `test/saveload.mjs` | 测试脚本 | 行为化 | — | — | — | — | **自证按需跑**：`node test/saveload.mjs --selftest`（故障注入＝落档后人为扰动，断言比较器判红）；不塞主链的理由＝自证需完整导航（成本≈主跑 30s，收益不值） |
| `test/saveui.mjs` | 测试脚本 | 行为化（缺自证） | — | — | ✅ | **临时下架**：样本随 demo 下架（对象＝该用例本身，属引擎/工具面判据）（until #1163（books 回填样本）） |  |
| `test/silent-gate.mjs` | 测试脚本 | 行为化（缺自证） | — | — | ✅ | — |  |
| `test/size-gate.mjs` | 测试脚本 | 行为化 | ✅ | — | ✅ | — |  |
| `test/state-diagnose.mjs` | 测试脚本 | 行为化 | ✅ | ✅ | ✅ | — |  |
| `test/story-ci.mjs` | 测试脚本 | 行为化 | ✅ | ✅ | ✅ | **临时下架**：用户故事 CI：K4/K5 面依赖故事与产物（until #1163（books 回填样本）） | **自证的归位（`#1056`）**：本件的 `--selftest` **不是入口** ✗ —— 它是**真断言的载荷**（`cli(['--selftest'])` 测壳的旗标面 ✓）⇒ 裸调与 `--selftest` 输出逐字节相同 ✓。⇒ **不接也不删**（`#1031` 口径 ✓）：硬接无意义旗标 ＝ 为凑绿而接线 ✗、删字符串 ＝ 拆真断言 ✗。真断言面由**裸调段**（`test-story-ci-mjs`）执行 ✓ —— `test-plan` 里那个 `-selftest` id 跑的就是裸调 ✓。 |

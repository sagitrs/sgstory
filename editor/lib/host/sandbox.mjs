...
//
//注意：**威胁模型（明文，不是默会）**：本沙箱只用来跑**本仓自己的故事脚本**，**不是**用来处理不可信输入的
...
import { absPath } from '../../../scripts/dist-paths.mjs';   // `#1267` 尾件①
 ...
 *注意：夹具注意：若夹具里出现 `Object.assign((window.Sg.story??= {}), …)`，缺 `window.Sg` 会落 catch → 返回 null
...
export const engineOf = (slug, fromPath = null) => {
	// `#1267` 尾件①：原写法**根本没走根**（`join(ROOT, 'stories/…')`）→ 外根下读仓内同名路径  。

...
//
//注意：**`#619` 第四条防线：`--story` 曾是被静默忽略的参数** —— 脚本全文没解析它，恒按 `DEFAULT_SLUG`（`mist-forest`）取文件集，
...
//
//注意：三条「假绿」防线（`#436` 原范围 3 补，`#557` 又补一条；都有自证钉住）：
...
const main = () => {
	...
	if (argv.includes('--selftest')) {
		...
		const cases = [
			...
			//注意：`#1004` B2b：这批用例原来**写死旧故事名**（`mist-forest`／`hollow-cave`）→ `DEFAULT_SLUG` 换成面夹具后：

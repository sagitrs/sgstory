...
判据（口诀）来自 `docs/criterion-design.md`（**唯一权威**）：能写成"你知道了……"→ ①；只是"发生过／已拥有"→ ②；
...
名字**全部取自既有词汇宏**（`damage`／`econ`／`give`／`take`）→ 符合 `#1049` 甲"具名动作、不造表达式面"的约束与 `docs/criterion-design.md` 的「别再造表达式面」。

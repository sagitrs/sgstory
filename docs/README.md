...
| 面 | 唯一权威 | 形态 |
|---|---|---|
| 知识模型（笔记 / 世界态 / 运行时） | `docs/criterion-design.md` | 手写 |
| 质量维度与每门判据 | `docs/criterion-design.md` | 手写 |
| **判据设计法则与流程语言**（总法则／术语／五条路／通用纪律／变更与归档） | **`docs/criterion-design.md`** | 手写（**术语与登记面指回代码** ✓） |
| 代码级约定（渲染路径 / 构建顺序 / 命名 / 条件表形状） | `docs/criterion-design.md` | 手写 |
| 引擎与故事的边界、故事接入契约 | `docs/criterion-design.md` | 手写 |
| 故事面范围（**必需／可选／不做**，编辑器暴露哪些面） | `docs/manual/12-data.md` | 手写 |

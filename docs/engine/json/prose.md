...
> ⚠️ **本节描述的是已裁定但尚未实现的目标形态**（甲-1）。**现状**：散文仍是 `.twee`（见本文 §5）。
...
|---|---|---|---|
| `passage` | `string` | ✅ | 段落名（＝链接目标、`scope` 的值）。⚠️ 与文件名**可以不同**（映射靠清单 `files`） |
| `tags` | `string[]` | — | 段落标签（`prose`／`script` 等；**非散文标签不判词汇门**） |
| `scope_of` | `string[]` | — | 可选：本段归属的条件行 id（供可达性视图） |

⚠️ **字段全集未定**（`decisions.md` 的 Q2）—— 上表是本文档给的最小集，不是裁定。
...
| ⚠️ **不判** | 注释跨度 `/% … %/` · `[script]`／`[widget]`／`[stylesheet]` 段 | 跳过 |

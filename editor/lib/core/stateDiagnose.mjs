...
//
//注意：**已抽 / 未抽**（件名声称的比现在交付的多 → 必须写明，免得下一个人以为漏了）：
...
//注意：该门的 **io 那一半留在门里**（`segmentsOf()` 吃 `readFileSync` 默认值 · `storyReadBaseline()` 吃 `loadStoryAudit`／`ROOT`）
// —— `lib/core/**` **不得碰宿主**（K6 ③）。
 ...
 *注意：`detail` 文案与搬家前**逐字相同**（门输出逐字节不变）；结构化 `field` 是**纯加法**（不打印，只给判据用）。
 */
 ...
 *注意：外部依赖只有 `flagPaths`／`keyOf` —— **两者已在 core** → 本函数**不需要**前置切片 `#881`。
 *注意：`detail` 文案与搬家前**逐字相同**（门输出逐字节不变）。 */
 ...
 *注意：`detail` 文案与搬家前**逐字相同**；曾经返回的 `where`（源文件名）**不再返回**（无人消费，门只打 `event`／`detail`）。 */
...
//注意：**io 那一半留在门里**：`segmentsOf()`（吃 `readFileSync` 默认值）／`storyReadBaseline()`
...
export const tableReadProblems = (rows) => {
	...
	for (const r of rows ?? []) {
		...
		for (const field of KEYS_OF) for (const k of asList(r[field]).flatMap(condKeysOf)) {
			...
			//注意：`#1156` **实测结论：此处不换 `readKeyFamily`** —— "**可读**（引擎能否取值）"与"**合法键形**（契约面口径）"

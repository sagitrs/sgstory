...
if (wantAll || arg('items') || arg('tokens')) {
	...
	let bad = 0; // 单一计数（此前经 `selfBad = bad` 二次转手 ⇒ 检测器与读者都看不出它就是被增量的那个）
	...
	if (process.argv.includes('--check') && (viol.length || bad)) { console.error(`\n✗ 道具/龙战矩阵段：${viol.length} 项不变量 + ${bad} 项自证`); process.exit(1); }
}
};

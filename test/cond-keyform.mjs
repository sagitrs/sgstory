...
//
//注意：注意：**本门刻意不复用审计层的规整逻辑**（这是本片最要紧的一处避免自盲）：
...
//
//注意：与审计层的**口径差异是已知的、且不在本门处置**：审计层宽容 `note:`（历史兼容），引擎不认 →
...
export const keyformProblems = ({ data, file = '(data)' }) => {
	...
	for (const pos of positionsOf(data)) {
		//注意：取键**必须逐项**调 `condKeysOf`（它对"数组里套对象"会退化成 `"[object Object]"` —— 实测）
 ...
 *
 *注意：注意：为什么不用 `readdirSync('stories')` 扫目录（**实测踩过**）：
 ...
 *注意：本仓**已有这教训**：`test/story-ci.mjs` 与 `editor/story-ci.mjs` 都写着「⊇ 而不是 ＝ …

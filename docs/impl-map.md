...
> **状态**：v2.1（**M1 骨架 + M2–M4 文案 + M5 终局 + M6a–f 打磨 + M5c–M5g 口径 + M7 v17 全案（去神秘化 / 传说层 / 好感合一 / 封印战）已完成**）
...
| `test/scenarios.mjs` | **24 条路线**（金路径 + 全部结局 + 设定集 + 龙巢边） | 重写 |
| `test/smoke.mjs` | 启动 → 快速车卡 → 酒馆 → 森林 → 洞穴 + 侧栏/存档/物品栏 | 重写 |
| `test/coverage.mjs` | 渲染/交互覆盖率 ratchet | **收紧为 5 门**（基线：渲染 **79** / 交互 **84**） |
...
**当前规模**：`npm test` 全绿（**24 条路线** · 内容段 69 · 渲染 79 · 交互 84 · 正文 ≈1.33 万字）；`npm run soak`（游走 20+20 局 + **18 位点**双支）0 违法，**已接回 CI 独立 job**（`deploy` 依赖 `[test, soak]`）。

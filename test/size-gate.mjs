...
for (const [k, v] of Object.entries(rows)) {
	...
	if (v > b + allow) { console.error(`✗ ${k}: ${v}B > 基线 ${b}B +容差 ${allow}B。确需增大：node test/size-gate.mjs --update-size 并在 PR 写明理由`); failures++; }
		...
		console.log(`✔ ${k}: ${v}B < 基线 ${b}B（-${b - v}）——已自动收紧`);
		if (!update) { base[k] = v; tightened = true; }
	}
...
if (tightened && !update) {
	const cur = existsSync(BASELINE) ? JSON.parse(readFileSync(BASELINE, 'utf8')) : { note: '', rows: {}, tolerancePct: {} };
	cur.rows = { ...cur.rows, ...Object.fromEntries(Object.entries(base).filter(([k]) => rows[k] !== undefined).map(([k]) => [k, rows[k]])) };
	writeFileSync(BASELINE, JSON.stringify(cur, null, '\t') + '\n', 'utf8');
	console.log('  （体积基线已自动收紧并写入——只紧不松；跨环境噪声由容差吸收）');
}

...
// ★ `#1447`（rebase 重放盖回 ⇒ 本轮补正）：`#568` 整段（28 格）**已搬 `test/rules-core.mjs`** ——
//   本件（**挂起段**）里那份是 rebase 把"前一轮的搬走"重放成主干版本留下的**第二份** ✗ ⇒ 删除。
//   ★教训（`#1112` 已入册）：**冲突解法不在提交内容里、在 rebase 过程里** ⇒ 每次 rebase 后要重核 ✓

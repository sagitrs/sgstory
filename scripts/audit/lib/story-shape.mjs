...
// 这里只做**静态形状校验**（gate-time）；引擎运行时的两条义务在 `docs/criterion-design.md` §八 8.9b：
...
// ★ `#1461`（第五刀）：**状态档位名**（`most`／`low`）与战斗档位（`crit`／`ok`／`bad`）
//   **同属「结果档位名」** ⇒ 已并入规则包的 `gradesAll`（一处权威 ✓）。
//   ★本表是**声明面校验词表**（`statuses.onFail` 的 `when` 只许这两个）⇒ 与包同源（✗ 零包时行为不变 ✓）。
export const GRADE_SET = ['most', 'low'];
/** 减成（`reduce`）允许的形态。故事声明"哪一种"，引擎按声明取值 → 这里就是**枚举权威**。
 *注意：**声明面不得大于实现面**（`#486` 切片②）：本表只列**引擎真的实现了**的形态 —— 引擎侧
 * `Game.Gear.slotAbsorb` 目前只落 `flat`（其余形态**大声报错**，不许静默 0 减成）。

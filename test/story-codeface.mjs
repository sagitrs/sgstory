...
//注意：到零后不许停在 `≤ 0`（形同虚设）→ 改成 `=== 0` 硬判
import { execFileSync } from 'node:child_process';
import { readFileSync } from 'node:fs';
import { join } from 'node:path';

const ROOT = new URL('..', import.meta.url).pathname.replace(/\/$/, '');
//注意：通配符**不要写进块注释** —— 其中的结束标记会提前终止注释（本文件第一版就栽在这）
const TWEE_GLOB = 'stories' + '/**/*' + '.twee';

/** 机制标签（**同义于** `editor/lib/core/text.mjs` 与 `scripts/audit/lib/shared.mjs` 各自那份）。
 *注意：仓内**已明文记录**这两处是「已知重复」且**刻意不统一**（`text.mjs` 顶部注释：导出会撞 K6、统一属另一件事 → 不在该片）
...
export const mechTagDrift = () => {
	...
	for (const f of files) {
		...
		//注意：**必须扫全部出现处** —— 只取第一处会漏：`shared.mjs` 里既有 `:134` 内联数组、

...
**判定口诀（评审补充，也是 `--notes` 门要自动查的东西）**：

> **a.** 能写成「你知道了……」一句**玩家可复述**的话 → **知识（笔记）**
> **b.** 只是「发生过／已拥有」 → **世界态**
> **c.** 只在同一次渲染/交互内有意义（去重、一次提示、战斗台账） → **运行时**
> **d.** 一个键若**同时**承担「发生过」与「知道」→ **拆成两个键**，别让它两用。

**按口诀已拆的两处**：`forest_listen`（我试过了＝世界态）／`forest_heard`（我听见了什么＝知识）——阶段 1 只收后者；`delivery_short`／`star_short` 归**运行时**（那是「这一次差了什么」的一次性提示）。

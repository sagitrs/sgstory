...
const { starterPackage } = await import('../editor/lib/core/story.mjs');
...
const pkg = newPackage({ doc: window.document, ifidOf: () => 'AAAA-BBBB-CCCC-DDDD' });
...
t('① 夹具 IFID 真的进了产物 ✓', /AAAA-BBBB-CCCC-DDDD/.test(pkg.twee['00-meta.twee']));
...
t('① 两条不同 IFID ⇒ `00-meta.twee` 逐字不同（照抄会撞 ✗）', (() => {
	const a = newPackage({ doc: window.document, ifidOf: () => 'IFID-A' });
	const b = newPackage({ doc: window.document, ifidOf: () => 'IFID-B' });
...
const pkg2 = newPackage({ doc: window.document, ifidOf: () => 'IFID-C', form: fakeForm });

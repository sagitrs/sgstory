...
//
//注意：`#1004` B2b 复核席**重做**（按裁定 A）：本件原先是**旧故事 `mist-forest` 的一条剧情走查**
...
//注意：**本件抓到 2 处夹具侧缺陷**（已上报，本片转绿前须修 —— 都不是本件的判据问题）：
...
//注意：夹具侧缺陷登记（见文件头注意：那两条）：这一段的 `$pc.classLabel$pc.bgLabel$pc.speciesLabel`
...
//（注意：夹具想叫"夹具旅人"而结果不是 —— 这是夹具侧的小缺陷，已一并上报，但**不影响本判据**）。

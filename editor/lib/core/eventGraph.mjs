 ...
 *
 *注意：**`inContractPaths` 只是弱提示** —— **不是"有没有登记"的判定**：
...
export const graphOf = ({ rows = [], members = [] } = {}) => {
	...
	for (const [id, ks] of Object.entries(needs)) {
		for (const k of ks) {
			...
			//注意：这两项都是**弱提示**（见件头）：命中 `contract.members` 的 `path`／`name` 面而已 —— 不构成"已/未登记"的判定。
	...
	return {
		...
		needs,
		grantedBy,
		/**「**事件声明面里**无人授予它」的项 ——注意：`inContractPaths`／`hasDefault` 都是**弱提示**：`contract.members` 的 `path`／`name` 面（note／flag 类的键登记在 **note 表**里 → 那面还没数据化 → 本件拿不到）。 */
		ungranted,
		counts: {
			events: byId.size,

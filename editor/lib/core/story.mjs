 ...
 *注意：所以本件**先归一化到大写再校验**（`crypto.randomUUID()` 出的就是**小写** → 页面路不归一化就永远过不了 `build`）。
 ...
 *
 *注意：**IFID 必须由调用方给**：`lib/core/**` **不得碰宿主**（不在 core 里取随机源）；
 * 页面用 `crypto.randomUUID()`、测试用夹具、CLI 将来可用 `node:crypto` —— 一处策略、多处注入。
 *注意：`entry` 必须与 `00-meta.twee` 里 `StoryData.start` **一致**（不一致 → 启始段找不到）。
 *注意：出参**只含源**：生成物由编译器产出 → 清单要用 `manifestFor()` 在**编译后**拼（`files` 里列的是**产物名**，与既有故事同形）。 */
export const starterPackage = ({ slug, title = '未命名故事', entry = '开场', ifid } = {}) => {
	...
	return {
		data: {
			//注意：形状**不是随便空的** —— 三件各有自己的最小合法形（与既有故事的数据面同形）：
 ...
 *注意：只列**真的写出去了**的件（漏列 → `build.mjs` 拒「故事件不在清单里」；多列 → 拒「清单里的文件不存在」）
...
export const manifestFor = ({ slug, title = '未命名故事', subtitle = '', entry = '开场', gates = [], twee = {}, audience = 'content' } = {}) => {
	...
	//注意：字段**总要写进清单**（不靠读侧补默认）；读侧 `audienceOf` 仍 fail-loud → 手工删掉字段会当场报错。

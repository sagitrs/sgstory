...
### 1.1 "空表"与"缺件"的判定差异（一句话版）

| 情形 | 结果 | 判定处 |
|---|---|---|
| **文件不在** | **抛错点名**："缺 `stories/<slug>/audit.json`：门侧的故事判据数据必须由**该故事自己**声明（空表也要显式写；`#602`）" | `scripts/audit/lib/story-audit.mjs:40`（`loadStoryAudit` 的 `existsSync` 分支） |
| **文件在，但 `text`／`topicWords`／`styleBlacklist` 不是数组** | 逐项点名（`judgeStoryAudit` 返回问题列表） | 同上 `:25-32` |
| **文件在，字段齐而值为空**（`{"text":{"topicWords":[],"styleBlacklist":[]},"readBaseline":{}}`） | **合法** → 文本门"本故事未声明主题词／违和词 → 本判据对该故事不适用，不报" | `judgeStoryAudit` 无问题 ＋ 门侧按空集合跳过 |

**为什么"缺件"要抛错而不是当空表**：`#602` 的旧洞是——**不给空表就等于借用别的故事的判据**（判据数据是**该故事自己的**，缺了不该静默继承）。
。

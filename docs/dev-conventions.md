...
| **规格门（MVP）** | `test/itemmatrix.mjs`（`npm run matrix`，`#640`）：**场景 × 道具/线索集合 → 期望**。数据住故事侧 `stories/<slug>/matrix.json`、门形状住引擎侧；
  ...
  三条判据＝①行键＝**谓词上下文** ②期望用**渲染后＋行为面**口径，且**分两相位**：`expects`＝渲染后·点击前、`expectsAfter`＝点击后（有 `click` 的行**必须**给 `lands`）；判据＝`text`/`noText`/`choice`/`noChoice`/`lands`/`delta`/`domCount`（`delta` 的 `null`＝键不存在＝消耗；`domCount`＝**结构面**，如 `[data-heard=...]` 折叠块） ③**承诺 ratchet**（`promised` 只增不减，每个原子两侧都要有行；承诺的原子在内容里消失即红）。当前承诺：**25 行 / 20 原子**（`顶楼`·`喂花`·`守林人`·`书房`·`老巫女`·`归位`·`女巫小屋`·`森林边缘`·`塔外花田`）；`npm run report:polarity` 的报告里同步标出「哪些原子已被门承诺」——**报告管抽样可见性、门管承诺**，两个口径并排看 |

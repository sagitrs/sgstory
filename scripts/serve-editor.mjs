...
// → 仓里**没有**任何一条能打开编辑器的路径。
//
//注意：**根必须是「仓根」**（本片最要紧的一条）：`editor/web/app.mjs` import 的是
 ...
 *注意：不能只 `join` 完就信任：`/../package.json` 会解析到根之外 → 必须**再核一次前缀**。 */
...
//注意：**只有以本文件为入口时才跑 CLI 分支**：否则 `import` 它会**直接起服务** → 测试件一 import 就挂住
...
if (isMain && process.argv.includes('--smoke')) {
	...
	const out = {
		...
		//注意：越界必须用**编码形**：`/../package.json` 会被 `fetch` 在客户端归一掉 → 服务端看到的是 `/package.json`（**根内**）

...
//
//注意：口径说明：本门是**结构判据**（读 `ci.yml` 的触发面与事件面），**不是**"跑一次真 draft→Ready"的
 ...
 *注意：排序（稳定输出 ＋ 可复现）：`readdirSync` 的次序不该决定报文次序 → 排一下。 */
 ...
 *注意：为什么必须都认：YAML **等价形态**不许造成判据缺口 —— 否则门会被"换个写法"**安静绕过 → 假绿**，
...
export const parseTypes = (prBlock) => {
	...
	if (m) {
		...
		for (const l of prBlock.slice(m.index + m[0].length).split('\n')) {
			if (!l.trim()) continue;                 //注意：空行不算列表结束（`types:` 之后的第一个元素前必有换行）
 ...
 *
 *注意：`#1087`：**返回 "该面在不在"** —— 以前只返回 `prTypes`，于是「**没有 `pull_request` 面**」与
...
export const triggerSurface = (text) => {
	...
	//注意：**块提取必须用行级扫描** —— 踩过：`onText.slice(idx).split(/^ {2}\S/m)[0]` 会**在开头就命中**
...
export const mainOnlyJobsMissingDispatch = (text) => {
	...
	for (let i = 0; i < lines.length; i++) {
		...
		if (m) job = m[1];
		//注意：`#1054` D1（复核）：**job 级与 step 级都要判** —— 只认 job 级会让 `upload-artifact` 的
		// step 级 `- if:` 改回只 `push` 时**静默放行**（实测 rc=0）→ 补跑时那步少跑一份产物。
		const ifl = (/^ {4,}if:\s*(.+)$/.exec(lines[i]) || /^ {4,}- if:\s*(.+)$/.exec(lines[i]));
		if (ifl && job) {
			//注意：`#1054` 复核：**引号归一化**后再判 —— YAML 里 `'push'` 与 `"push"` **等价**，
...
/** 判据总入口（**纯函数** → 可喂合成文本单测）。
 *
 *注意：`#1087` 裁定：**判据条件化**（**同一判据 ＋ 适用面写准**，**不新造**）——
 * 各条判据**只对它声称的那个面**发言：
...
if (process.argv.includes('--selftest')) {
	...
	t('反例⑥（复核给的 A5 形态）：main-only 不接受 dispatch 且用**双引号** `"push"` ⇒ **仍点名**（不许被等价写法绕过 ✗）', (() => {
		...
		//注意：不写死条数（夹具里同类 job/step 可能多于一个）→ 要求「至少一条、且点到 deploy」
	...
	//注意：风险：把"条件化"写成"**宽松化**" → 真判据被放过（门形同虚设）。→ 三个方向都要能假。
	const prOnly = `on:\n  push:\n    branches: [main]\n  workflow_dispatch: {}\njobs:\n  a:\n    runs-on: ubuntu-latest\n`;
	...
	//注意：为什么必须靠自证格：**③"射程被收窄"在 CI 里不会自然发生** → CI 绿**不能**证明本判据对
	// → 用纯函数喂夹具钉住（不依赖真树）。

...
**★ 三门账（#1353 ①）**：**门 43**（有自证的行为化） ｜ **检视 18**（仅登记 ＋ **临时暂缓** —— 人来做、留读数） ｜ **欠账 42**（行为化缺自证 37 ＋ **有探针件却探不到** 5）
>
**严格行为化率（有自证）：43/81 = 53.1%** ｜ **有断言但缺自证：37**（＝下方工作清单）｜ 仅登记：0
**探针（直接读数 ✓，不是"文件在不在"那种代理 ✗）：`✅` 12 项 ｜ `—` 未探 69 项（**上限 117** ✓ 超过即红 ✗；**调高它**是一次显式手改 ⇒ 靠评审拦 ✗，机器拦不住“手改上限”本身 ✓ —— 边界记在票 #908 内 ✗）｜ `✗` 不咬 0 项（**>0 即红** ✓）** —— 档位／清单：`node scripts/probe-gates.mjs --probe=fast` ✓（⑲：本轮覆盖到哪一档写在这行里 ✓）
...
| `test/chargen-shape.mjs` | 测试脚本 | 行为化（缺自证） | — | — | ✅ | **临时下架**：车卡面判据（`#1353` 乙组拆出）；夹具缺 `chargen` 数据面 ＋ 引擎段序缺陷 `#1418`（until #1418 修好后：接夹具 m3-chargen-fixture 并撤挂） |  |

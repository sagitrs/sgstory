...
> **状态（`#1325`，2026-09-24）**：**史料 —— 编辑器产品线（`editor/web`：WebUI／编辑器 UI）已随 `#1261` 大裁剪砍掉** ⇒
> **✗ 不要再照本手册去执行"翻面"**。保留原因：它是两次已完成翻面（`minimal-demo`／`hollow-cave`）里抽出的流程
> 与踩过的坑，其中"**冻基线 → 门重指向 → 产物 → 双向反例 → 重签**"这套**手法**仍有参照价值。
> **现形态与约定以 `docs/engine/**` 与引擎实现为准；`editor/lib/**` 与 `editor/*.mjs`（编译器／门）仍在交付面上。**
...
> 免得下一个故事重踩）。注意：**`#1004` B 段后**：当时"第三个故事"（`mist-forest`）**已删** → 现存的真内容故事是 `night-ferry`（P4 用编辑器做出）、另有测试夹具 `face-fixture`；将来的用户故事照本手册做即可。
